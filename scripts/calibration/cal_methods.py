...
class TemperatureScaling():
    ...
    def __init__(self, temp = 1, maxiter = 50, solver = "BFGS", attended_nll = False, theta1 = 0.995, theta2 = 0.999):
        """
        Initialize class
        ...
        """
        self.temp = temp
        print('First Temp is: ', self.temp)
        self.maxiter = maxiter
        self.solver = solver
        self.attended_nll = attended_nll
        self.theta1 = theta1
        self.theta2 = theta2
    ...
    def _loss_fun(self, x, probs, true):
        ...
        if not self.attended_nll:
            loss = log_loss(y_true=true, y_pred=scaled_probs)
        else:
            one_hot_true = np.zeros((true.shape[0], true.max()+1))
            one_hot_true[np.arange(true.shape[0]),true] = 1
            alphas = one_hot_true.sum(axis=0, keepdims=1) / (one_hot_true.shape[0] - one_hot_true.sum(axis=0, keepdims=1))
            # print(one_hot_true.sum(axis=0, keepdims=1))
            # print(alphas)
            m = scaled_probs/(1 - scaled_probs)
            n = m * alphas
            theta1s = self.theta1 * alphas
            theta2s = self.theta2 * alphas
            # print(theta1s)
            # print(theta2s)
            loss = 0
            fail_count = 0
            for i in range(one_hot_true.shape[0]):
                for j in range(one_hot_true.shape[1]):
                    if (one_hot_true[i, j] == 1 and n[i, j] > theta1s[0,j]) or (one_hot_true[i, j] == 0 and n[i, j] < theta2s[0,j]):
                        fail_count += 1
                        continue
                    loss += -np.log(n[i,j])
            # o = -np.log(n)
